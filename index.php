...
$sy_qry = $conn->query("SELECT * FROM `school_year` where `status` = 1 ");
$result = $sy_qry->fetchArray();
if(isset($result['sy_id'])){
    $_SESSION['sy_id'] = $result['sy_id'];
    $_SESSION['sy'] = $result['school_year'];
}else{
    $_SESSION['sy_id'] = 0;
    $_SESSION['sy'] = "";
}
    ...
    <main>
        ...
        <div class="container">
            <div class="py-1 w-100 d-flex justify-content-end">
                <small class="text-muted">Active School Year: </small><span class="badge bg-success mx-2 rounded-pill"><?= $_SESSION['sy'] ?></span>
            </div>
        </div>
